...
static const unsigned int _BAUDRATE = 115200;  //for software serial

//Ultrasonics sensor
const int trigPin = 9;
const int echoPin = 10;
long duration;
int distance, set_distance = 14; //set distance is 14cm

//define pins for L293D Motor controller
const int Left1 = 40;  // L293D Pin2  >> Arduino Pin40
const int Left2 = 41;  // L293D Pin7  >> Arduino Pin41
const int Right1 = 42; // L293D Pin10 >> Arduino Pin42
const int Right2 = 43; // L293D Pin15 >> Arduino Pin43
...
{
   // Read captured frame from FIFO memory and send each byte as it is to serial port via software serial
   ...
   Serial.println(F("Starting Transmission of Photo To Serial Port ..."));
     ...
     for (int width = 0; width < PHOTO_WIDTH; width++)
     {
       for (int bytenumber = 0; bytenumber < PHOTO_BYTES_PER_PIXEL; bytenumber++)
       {
         ...
         PixelData = PinVal7 | PinVal6 | PinVal5 | PinVal4 | PinVal3 | PinVal2 | PinVal1 | PinVal0;
          ByteCounter = ByteCounter + mySerial.write(PixelData);       
        
       }
     }
   }
   
   Serial.print(F("Total Bytes Sent For Recent Frame: "));
...
void TakePhoto()
{
  // Take Photo using the ov7670 camera and send frame to serial port
...
void getDistance() {
  ...
  distance = duration * 0.034 / 2; //cm unit
...
void setup()
{
     ...
     pinMode(Left1, OUTPUT); // Sets the Left1 as an Output
     pinMode(Left2, OUTPUT); // Sets the Left2 as an Output
     pinMode(Right1, OUTPUT); // Sets the Right1 as an Output
     pinMode(Right2, OUTPUT); // Sets the Right2 as an Output
...
{   
     // Take Photo
         ...
         Serial.println(F("Photo Taken and sent to Serial Port..."));
         PhotoTakenCount++;

     //getting response from serial port matlab    
     while (1)
     {
       if (Serial.available() > 0) 
       {
         int NumberCharsRead = Serial.readBytesUntil('!', IncomingByte, BUFFERLENGTH);  
         for (int i = 0; i < NumberCharsRead; i++)
         {
           Direction += IncomingByte[i];
         } 
         break;
       }
     }
       ...
       if(Direction=="forward"){
           getDistance();
          if (distance>=set_distance){  //change set distance according to results
            moveforward();
            delay(400);     //set delay according to moving speed of robot.
            stoprobot();
            }
       }else if(Direction=="backward"){
          if (distance<=set_distance){  //set distance
              movebackward();
              delay(400);     //set delay according to moving speed of robot.
              stoprobot();
            }
              ...
              turnright();
              delay(150);     //set delay according to moving speed of robot.
              stoprobot();
